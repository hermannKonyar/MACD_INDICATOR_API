...
def check_coin(context: CallbackContext):
    ...
    # MACD ve Stochastic RSI hesaplama
    macd, macd_signal, _ = talib.MACD(close_prices)
    rsi = talib.RSI(close_prices)
    rsi_stoch = talib.STOCHRSI(rsi)

    # MACD ve Stochastic RSI değerlerini kontrol etme
    last_macd_histogram = macd[-2] - macd_signal[-2]
    current_macd_histogram = macd[-1] - macd_signal[-1]
    current_rsi_stoch = rsi_stoch[-1]

    # Alım, satım veya bekle mesajı oluşturma
    message = ""
    if last_macd_histogram < 0 and current_macd_histogram > 0 and current_rsi_stoch < 0.2:
        message = f"ALIM: ETHTRY - Fiyat: {current_price}"
    elif last_macd_histogram > 0 and current_macd_histogram < 0 and current_rsi_stoch > 0.8:
...
def main():
    ...
    job_queue.run_repeating(check_coin, interval=600, first=0)
